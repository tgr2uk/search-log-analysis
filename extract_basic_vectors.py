...
# TODO
# Extend output to deliver full vectors for each session
...
def determineSessionType():
    ...
    logging.warning('%d,%d,%d', numQueries, numClicks, numPages)
...
tPrevious = datetime.datetime.strptime("2006-03-01 07:17:12", format)
idPrevious = '142'
...
totQueryTerms = totQueries = totClicks = totPages = 0

# print "Time now is :", datetime.datetime.now().strftime("%A, %d. %B %Y %I:%M%p")
startTime = time.time()
...
with open(args.inputFile) as f:
    for line in f:
        ...
        if (delta >= 30) or (id != idPrevious):   # it's the end of a session
            (totQueries, totClicks, totPages) = determineSessionType()
